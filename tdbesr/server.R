...
shinyServer(function(input, output, session) {
    ...
    output$etu.norm <- renderPlot({ plots()$etu.norm + th_norm + ggtitle("Valeurs normalisées") + cpesr_cap() }, res= fig.res)
    ...
    output$bia.norm <- renderPlot({ plots()$bia.norm + th_norm + ggtitle("Valeurs normalisées") + cpesr_cap() }, res= fig.res)
    ...
    output$ens.norm <- renderPlot({ plots()$ens.norm + th_norm + ggtitle("Valeurs normalisées") + cpesr_cap() }, res= fig.res)
    ...
    output$fin.norm <- renderPlot({ plots()$fin.norm + th_norm + ggtitle("Valeurs normalisées") + cpesr_cap() }, res= fig.res)
    output$fin.evol <- renderPlot({ plots()$fin.evol + th_evol + ggtitle("Evolutions") }, res= fig.res)

    output$serie <- renderPlot(res= fig.res, { 
      kpiESR::kpiesr_plot_line(id = input$etab, vars = input$serie_vars, val = input$serie_val) + cpesr_cap()
    })
